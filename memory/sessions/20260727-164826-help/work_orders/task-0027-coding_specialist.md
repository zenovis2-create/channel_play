...
Goal: Recover an ambiguous Truth Pen owner-save response without automatically retrying the write. Generate a cryptographically random apply-attempt ID in the browser, reserve it before the atomic save, and retain only a bounded, short-lived, value-redacted result in server memory. Expose a protected strict-schema status endpoint so the browser can recover a completed result after a lost or malformed primary response. Preserve the existing one-time preview grant, exact confirmation phrase, two checkboxes, no-op block, atomic write, post-write verification, and separate PASS-receipt gate. Never persist answer values, authorize contact, create a receipt, use browser storage, or weaken execution gates. Add result-store and protected API tests, UI response-loss recovery contracts, JavaScript syntax, Chrome preview-only proof with zero apply/status calls, unchanged real hashes, focused/full suites, runtime receipt, and findings-first security review.
Read first: agents/company.md, agents/memory_policy.md, memory/company/current_brief.md, memory/company/task-0027-plan.md
Allowed write paths:
...
Inputs: memory/company/task-0027-plan.md, task request, and current brief
Expected output: changed files or report matching role contract
Verification required: bounded and redacted result-store tests, protected exact-schema status API tests, response-loss recovery UI contracts, JavaScript syntax, Chrome preview-only proof with zero apply/status requests, unchanged real manifest and receipt hashes, focused and full Python suites, runtime receipt, and findings-first security review
